...
private:
    ...
    void simulateScorers(Team * t, std::list<Player *> & l, int g) {
        for (int i = 0; i < g; ++i) {
            // Genera un índice aleatorio dentro del rango de jugadores del equipo
            int ix = rand() % t->getPlayers().size();

            // Obtén un iterador que apunte al jugador en la posición aleatoria
            std::list<Player *>::iterator it = t->getPlayers().begin();
            std::advance(it, ix);

            // Agrega el jugador al final de la lista de goleadores
            l.push_back(*it);
        }
    }
